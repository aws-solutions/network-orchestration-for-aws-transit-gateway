...
def cfn_handler(event, context, create, update, delete, logger, init_failed):
    ...
    # returned physical id changes, cloudformation will then issue a delete
    # against the old id
    ...
    if init_failed:
        send(event, context, "FAILED", response_data, physical_resource_id, logger, reason="Initialization Failed")
    ...
    try:
        ...
        send(event, context, "SUCCESS", response_data, physical_resource_id, logger=logger)

    # Catch any exceptions, log the stacktrace, send a failure back to
    # CloudFormation and then raise an exception
    except Exception as e:
        logger.error(e, exc_info=True)
        send(event, context, "FAILED", response_data, physical_resource_id, reason=e, logger=logger)
    finally:
        t.cancel()
