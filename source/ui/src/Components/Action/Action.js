...
import {getActionItemsFromTransitNetworkOrchestratorTables,getVersionHistoryForSubnetFromTransitNetworkOrchestratorTables} from'../../graphql/queries'; 
...
class Action extends Component { 
  ...
  constructor(props) { 
    ...
    getActionItems=async () =>{ 
      const result=await API.graphql(graphqlOperation(getActionItemsFromTransitNetworkOrchestratorTables)); 
      this.setState({items:result.data.getActionItemsFromTransitNetworkOrchestratorTables.items});  
    } 
    ...
    processRequest= async adminAction =>{ 
      try{ 
            ...
            const input={SubnetId:this.state.selectedSubnetId,Version:"latest",Status:"processing",UserId:this.state.currentUser,GraphQLTimeStamp:UTCTimeStamp, AdminAction:adminAction}; 
 
            await API.graphql(graphqlOperation(updateTransitNetworkOrchestratorTable,{input})); 
    ...
    onRowSelected= params => { 
      ...
      { 
          ...
          if (this.state.currentGroup.includes("AdminGroup") && selectedRows[0].Status !== "processing"){ 
              this.setState({ 
        ...
        return ( 
          ...
          > 
              <div className="dropdown"> 
                  <button className="btn btn-light dropdown-toggle" style={{background: "#5d9cd2", color: "white", fontSize:"10pt", margin:"5px"}} type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> 
                    Action 
                  </button> 
                  <div className="dropdown-menu" aria-labelledby="dropdownMenuButton" style={{background: "#e1e4e9", color: "black",fontSize:"10pt"}}> 
                    <button id="btn-accept" className="dropdown-item" disabled={this.state.btnAdminActionDisabled} onClick= {()=> this.setState({showAcceptConfirmationModal:true})}>Approve</button>  
                    <ConfirmAcceptModal show={this.state.showAcceptConfirmationModal} onHide={this.handleAccept.bind(this)} params={{action:"accept",selectedRow:this.state.selectedRow}}/> 
                    <button id="btn-reject" className="dropdown-item"  disabled={this.state.btnAdminActionDisabled} onClick= {()=> this.setState({showRejectConfirmationModal:true})}>Reject</button> 
                    <ConfirmRejectModal show={this.state.showRejectConfirmationModal} onHide={this.handleReject.bind(this)} params={{action:"reject",selectedRow:this.state.selectedRow}}/> 
                     <button id="btn-history" className="dropdown-item"  disabled={this.state.btnHistoryDisabled} onClick= {() =>this.showVersionHistory()}>View History</button>  
                ...
                <div className="divright" > 
                  <button id="btn-refresh-action" style={{background: "#5d9cd2", color: "white"}} onClick= {() =>this.getActionItems()}> 
                    <FaSyncAlt/> 
                  </button> 
                </div> 
              </div> 
